...
resource "helm_release" "postgresql" {
  name       = "postgresql"
  # 【重大变更教训】Bitnami 从 2025 年起将 Helm Chart 仓库迁移到了 OCI (Open Container Initiative) 格式。
  # 旧的 HTTPS 仓库地址 "https://charts.bitnami.com/bitnami" 的 index.yaml 现在指向 OCI 引用，
  # 导致 Terraform 的 Helm Provider 在下载 Chart 时报 "invalid_reference: invalid tag" 错误。
  # 解决方案：使用 OCI 格式直接引用 Chart，不再使用 repository 参数。
